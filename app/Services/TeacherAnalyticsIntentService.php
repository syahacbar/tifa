...
class TeacherAnalyticsIntentService
{
    ...
    public function parse(string $question, ?array $context = null): ?array
    {
        ...
        if (! preg_match('/\b(berapa|jumlah|sebaran|bandingkan|tampilkan|mana|sebutkan|terbanyak|top)\b/u', $text) || ! $this->hasTeacherTerm($text)) return null;
        ...
        $group = null; $top = null;
        if (preg_match('/(per|berdasarkan)\s+distrik/u', $text) || preg_match('/\b(?:distrik|daerah)\s+mana\b/u', $text) || (str_contains($text, 'distrik') && (str_contains($text, 'terbanyak') || str_contains($text, 'paling banyak')))) $group = 'district';
        ...
        $metric = preg_match('/\b(penugasan|assignment|record nominatif)\b/u', $text) || $this->hasTeachingPersonnelTerm($text) ? 'assignment_count' : 'unique_teacher_count';
    ...
    private function hasTeachingPersonnelTerm(string $text): bool
    {
        return preg_match('/\b(?:tenaga\s+pengajar(?:nya)?|para\s+pengajar|pengajar(?:nya)?|pendidik(?:nya)?|tenaga\s+pendidik(?:nya)?)\b/u', $text) === 1;
    }
}
